...
def test_committed_default_references_the_reviewed_plan_of_the_failed_deployment():
    """The pointer names the plan emitted for run 35427436860 attempt 1 — the
    #38 apply whose L1 verify exited 1 — not the earlier :18 plan."""
    plan_id = load_active_plan_id()
    plan = load_plan(plan_id)
    assert plan_id == "l1-35427436860-1"
...
def test_recovery_requires_the_exact_maintenance_state_before_restoring():
    ...
    plan = load_plan("l1-35427436860-1")
    ...
    assert targets["plan_id"] == "l1-35427436860-1" and targets["fleets"]["l0"]["desired_count"] >= 1
