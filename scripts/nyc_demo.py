"""NYC demo (HLD v2 §5 acceptance): "describe your organization" → blueprint,
...
L6 tests use so the layers have something to narrate.
    ...
    python scripts/nyc_demo.py
    python scripts/nyc_demo.py --synthetic-corpus --e2e --artifacts artifacts/nyc-demo
    python scripts/nyc_demo.py --base-url https://clhear.org --api --e2e
"""
...
class Demo:
    ...
    def run_api(self) -> dict:
        ...
        build = self.call("POST", "/solon/build", {"attributes": state["attributes"], "name": "NYC demo · global retail broker"})
...
def seed_synthetic_corpus(db: Path) -> None:
    """Migrate the demo DB and load the offline UK corpus through L1→L5 (no network, FakeProvider)."""
    import tempfile
...
def start_app(port: int, synthetic_corpus: bool = False) -> subprocess.Popen:
    ...
    env = {**os.environ, "CLHEAR_HTTP_MODE": "replay", "DATABASE_URL": f"sqlite:///{db}", "PYTHONPATH": str(ROOT)}
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="CLHEAR NYC demo: describe → blueprint, end to end")
    ...
    ap.add_argument("--synthetic-corpus", action="store_true",
                    help="load the offline UK test corpus into the self-started app (ignored with --base-url)")
    ...
    try:
        ...
        if args.e2e:
            try:
                ...
                return 2
            out["e2e"] = demo.run_e2e(Path(args.artifacts) if args.artifacts else None)
    finally:
        if proc is not None:
            proc.terminate()
            try:
