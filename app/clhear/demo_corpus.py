...
def derive_demo(engine) -> dict:
    """One-shot L2–L5 derivation for the demo sources already in L1.

    L6 is composed per request. Does not start a cycle, does not score L7 from
    these posts, and does not lift the downstream hold for every publisher.
    NIST CSF is included when an in-force version is already stored; it is a
    control catalog, not an advertising code, and it contributes an obligation
    only when a duty is present.
    """
    ...
    return {"extracted": extracted, "predicates": stamped, "decomposed": decomposed, "mapped": mapped,
            "l7": "enforcement exposure, not scored from posts", "l8": "locked"}
...
def demo_status(engine) -> dict:
    ...
    from app.clhear.l7.models import enforcement_links, risk_scores
    ...
    with engine.connect() as conn:
        ...
        links = conn.execute(sa.select(sa.func.count()).select_from(enforcement_links).where(
            enforcement_links.c.obligation_id.in_(obligation_ids), enforcement_links.c.valid_to.is_(None))).scalar_one()
        scores = conn.execute(sa.select(sa.func.count()).select_from(risk_scores).where(
            risk_scores.c.subject_ref.in_(obligation_ids))).scalar_one()
    ...
    return {
        ...
        "L7": {"enforcement_links": links, "risk_scores": scores},
        "L8": {"status": "locked"},
    }
