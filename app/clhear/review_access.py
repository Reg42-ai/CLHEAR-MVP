...
<script>fetch('/auth/me').then(r=>r.json()).then(d=>{const p=d.providers||{};for(const [k,label] of [['cognito','Organization sign in'],['google','Google']]){if(p[k]){const a=document.createElement('a');a.href='/auth/'+k;a.textContent=label;document.getElementById('providers').append(a);}}});
