...
resource "aws_iam_role_policy" "webui_db" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        # Runtime valueFrom equivalent: hydrate INFER_TOKEN and session secrets from SSM.
        Effect   = "Allow"
...
data "aws_ssm_parameter" "sentry_dsn" {
  count      = local.deploy_webui ? 1 : 0
  name       = aws_ssm_parameter.sentry_dsn.name
...
resource "aws_lambda_function" "webui" {
  ...
  environment {
    variables = {
      ...
      CLHEAR_APP_KEYS                 = "" # the web service reads /clhear/web/CLHEAR_APP_KEYS (SSM)
