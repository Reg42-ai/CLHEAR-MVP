...
- **AI Ops ( #/ops )** — chronological feed of router decisions, fleet writes,
  revalidation verdicts, gate outcomes — each with a reasoning field.
...
did not happen shows as `schedule-missed` and trips an alarm). Inference is
remote — Reg42 Infer on Amazon Bedrock, procurement-clean ladders per task
class, model ids frozen per release. Then the AI fleets run: L2 duty-triage (evidence-span contract) and
...
cheapest sufficient rung of the task-class ladder; premium rungs are capped
